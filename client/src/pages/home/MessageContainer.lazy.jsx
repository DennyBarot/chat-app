...
import { getMessageThunk, loadMoreMessagesThunk, markMessagesReadThunk } from "../../store/slice/message/message.thunk";
...
const MessageContainer = ({ onBack, isMobile }) => {
  ...
  const pagination = useSelector((state) => state.messageReducer.pagination || { hasMore: false, cursor: null });
  ...
  useEffect(() => {
    const loadMessages = async () => {
      if (selectedUser?._id && location.pathname !== '/login' && location.pathname !== '/signup') {
        await dispatch(getMessageThunk({ otherParticipantId: selectedUser._id }));
      }
    };
    loadMessages();
  }, [selectedUser, location, dispatch]);
  ...
  useEffect(() => {
    const markAsRead = async () => {
      if (selectedConversationId && socket) {
        ...
        await dispatch(markMessagesReadThunk({ conversationId: selectedConversationId }));
      }
    };
    markAsRead();
  ...
  useEffect(() => {
    if (!messagesContainerRef.current || !pagination.hasMore) return;

    const observer = new IntersectionObserver(
      (entries) => {
        const firstEntry = entries[0];
        if (firstEntry.isIntersecting && !isLoadingMore && pagination.hasMore) {
          loadMoreMessages();
        }
      },
    ...
    const loadMoreMessages = async () => {
      if (selectedUser?._id && pagination.cursor && !isLoadingMore) {
        setIsLoadingMore(true);
        await dispatch(loadMoreMessagesThunk({
    ...
    const target = messagesContainerRef.current?.firstChild;
    if (target) {
  ...
  }, [selectedUser, pagination.hasMore, pagination.cursor, isLoadingMore, dispatch]);
      ...
      {selectedUser ? (
        <>
          ...
          <div 
            ...
            className="flex-1 overflow-y-auto p-4 bg-gradient-to-b from-slate-50 to-white dark:from-slate-800 dark:to-slate-900"
          >
            {isLoadingMore && (
                  ...
                  if (item.type === "date-separator") {
                    ...
                    return (
                      <Message
                        ...
                        onReply={handleReply}
                        isLastMessage={index === messagesWithSeparators.length - 1}
                      />
                    );
                  }
                  return null;
