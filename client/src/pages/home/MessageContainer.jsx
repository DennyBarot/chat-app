...
const MessageContainer = ({ onBack, isMobile }) => {
  ...
  const handleCall = useCallback((callType) => {
    if (selectedUser) {
      initiateCall(selectedUser._id, callType);
    }
  }, [selectedUser, initiateCall]);
        ...
        <>
          <div className="p-4 border-b border-foreground shadow-sm flex justify-between items-center">
            ...
            <div className="flex space-x-2">
              <button onClick={() => handleCall('audio')} className="p-2 bg-green-500 text-white rounded-full hover:bg-green-600">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
              </button>
              <button onClick={() => handleCall('video')} className="p-2 bg-blue-500 text-white rounded-full hover:bg-blue-600">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
